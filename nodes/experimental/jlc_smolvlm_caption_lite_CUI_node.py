"""
JLC CaptionForge SmolVLM (Lite) — ComfyUI Node Wrapper
...
- Node Purpose
    - The **JLC CaptionForge SmolVLM (Lite)** node provides a compact ComfyUI
      frontend for SmolVLM-family image captioning inside CaptionForge.
    ...
    - This file is the **ComfyUI-facing wrapper**, not the reusable captioning
      engine. It is responsible for:
            ...
            • passing user settings into the shared SmolVLM caption engine

    - The actual reusable captioning implementation lives in:
            jlc_smolvlm_caption_engine.py

      That engine handles:
            • SmolVLM model registry lookup
...
- CaptionForge Pipeline Role
    ...
    - The SmolVLM Lite node contributes compact SFW-oriented caption evidence
...
- Node Workflow Model
    ...
    - In standalone mode, the node behaves as a compact direct-IMAGE captioning
      node and writes to:
            ComfyUI/output/jlc_captionforge_smolvlm_lite/
    ...
    - When connected to the CaptionForge Pipeline Planner, this node consumes
      ...
      uses the SmolVLM model-family plan to determine:
            • whether SmolVLM captioning participates in the run
            • how many SmolVLM raw-caption witnesses to generate per image
...
- Model and Dependency Notes
    - SmolVLM repositories are multi-file Hugging Face model directories, not
      single checkpoint files.

    - Runtime behavior depends on the active ComfyUI Python environment,
      PyTorch, Transformers, Accelerate, Pillow, and Hugging Face Hub
      dependencies.
...
- Design Philosophy
    ...
    - CaptionForge is engine-democratic: SmolVLM captions contribute evidence to
      the broader audit and consensus pipeline alongside Joy, Qwen, future local
...
- Attribution & License
  ...
  - SmolVLM-family model loading is designed around compatible Hugging Face
    Transformers interfaces and publicly available SmolVLM-family checkpoints.
...
MANIFEST = {
    "name": "JLC CaptionForge SmolVLM (Lite)",
    "version": (0, 3, 0),
    "author": "J. L. Córdova",
    "description": (
        "Compact CaptionForge frontend for SmolVLM-family image captioning. Provides "
        "direct IMAGE captioning while also consuming CAPTIONFORGE_PIPELINE_PLAN objects "
        "from the CaptionForge Pipeline Planner through the pipeline_plan input. Template "
        "extras are no longer duplicated as local widgets; they are consumed only through "
        "the template_options input from the CaptionForge Template Options sidecar. The UI "
        "now separates caption_template_mode and custom_prompt_mode for clearer prompt routing "
        "while delegating reusable SmolVLM-family model loading, generation, cleanup, cache "
        "integration, and audit-record creation to jlc_smolvlm_caption_engine.py."
    ),
}
...
class JLC_SmolVLMCaptionLite:
    """Compact SmolVLM witness node for CaptionForge."""
            ...
            "required": {
                "model": (
                    list(MODEL_REGISTRY.keys()),
                    {
                        ...
                        "tooltip": (
                            "Select the SmolVLM-family vision-language model. Models are loaded from "
                            "ComfyUI/models/LLM/JLC_SmolVLMCaption/."
                        ),
                    },
                ),
                "memory_mode": (
                    ...
                    {
                        "default": "Default",
                        "tooltip": "SmolVLM model memory mode.",
                    },
                ),
                "keep_loaded": (
                    "BOOLEAN",
                ...
                "caption_type": (
                    CAPTION_TYPE_CHOICES,
                    {
                        "default": "SFW Character Caption" if "SFW Character Caption" in CAPTION_TYPE_CHOICES else CAPTION_TYPE_CHOICES[0],
                ...
                "prompt_preset": (
                    ...
                    {
                        "default": "detailed" if "detailed" in PROMPT_PRESETS else list(PROMPT_PRESETS.keys())[0],
                ...
                "num_beams": (
                    "INT",
                    {
                        ...
                        "max": 16,
                        "step": 1,
                        "tooltip": "Standalone beam count for SmolVLM generation.",
                    },
                ),
                "temperature": (
                    "FLOAT",
    ...
    def caption(
        ...
        if run_plan_connected and not run_plan:
            status = "[CaptionForge] SmolVLM Caption Lite disabled by Pipeline Planner."
            print(status)
        ...
        cfg = SmolVLMCaptionConfig(
            model_name=model,
            model_path="",
            model_root=str(JLC_SMOLVLM_MODEL_ROOT),
        ...
        file_images = (
            _iter_input_path_images(first_run.input_path, first_run.recursive, first_run.filename_glob)
        ...
        output_dir = Path(first_run.output_dir) if first_run.output_dir else Path(folder_paths.get_output_directory()) / "jlc_captionforge_smolvlm_lite"
        ...
        if run_plan_connected:
            write_run_config_json(
                output_dir / f"jlc_captionforge_smolvlm_lite_run_config_{timestamp()}.json",
        ...
        def process_one(source_name: str, pil: Image.Image):
            for run in run_plan:
                ...
                engine.config.prompt = prompt

                t0 = time.perf_counter()
                final_caption, raw_caption = engine.caption_pil(pil)
                dt = time.perf_counter() - t0
                print(f"[JLC CaptionForge SmolVLM Lite] Generation time run {run.ensemble_run_index}: {dt:.2f}s")
                ...
                print(
                    f"[JLC CaptionForge SmolVLM Lite] Captioned {source_name} "
...
NODE_CLASS_MAPPINGS = {
    "JLC_SmolVLMCaptionLite": JLC_SmolVLMCaptionLite,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "JLC_SmolVLMCaptionLite": "\u2003JLC CaptionForge SmolVLM (Lite)",
}
