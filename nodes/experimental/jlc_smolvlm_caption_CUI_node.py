"""
JLC SmolVLM Caption — ComfyUI Node Wrapper
"""
from __future__ import annotations

MANIFEST = {
    "name": "JLC SmolVLM Caption",
    "version": (0, 2, 0),
    "author": "J. L. Córdova",
    "description": "ComfyUI frontend for SmolVLM-family CaptionForge Pass A captioning.",
}
...
from ..engines.jlc_smolvlm_caption_engine import (
    BatchCaptionConfig,
    CaptionRecord,
    CleanupConfig,
    SmolVLMCaptionConfig,
    SmolVLMCaptionEngine,
    GenerationConfig,
    MEMORY_MODES,
    MODEL_REGISTRY,
    PROMPT_PRESETS,
...
JLC_SMOLVLM_MODEL_ROOT = Path(folder_paths.models_dir) / "LLM" / "JLC_SmolVLMCaption"
...
class JLC_SmolVLMCaption:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "model": (list(MODEL_REGISTRY.keys()), {"default": "SmolVLM-Instruct", "tooltip": "Select model."}),
                ...
                "prompt_mode": (["custom_or_preset", "caption_template"], {"default": "caption_template", "tooltip": "custom_or_preset uses custom_prompt or prompt_preset. caption_template uses caption_type, caption_length, and extra options."}),
                "prompt_preset": (list(PROMPT_PRESETS.keys()), {"default": "detailed"}),
                "caption_type": (CAPTION_TYPE_CHOICES, {"default": "SFW Character Caption" if "SFW Character Caption" in CAPTION_TYPE_CHOICES else CAPTION_TYPE_CHOICES[0]}),
                "caption_length": (CAPTION_LENGTH_CHOICES, {"default": "any"}),
                "extra_option1": (CAPTIONFORGE_EXTRA_OPTIONS_CHOICES, {"default": ""}),
                "extra_option2": (CAPTIONFORGE_EXTRA_OPTIONS_CHOICES, {"default": "", "advanced": True}),
                "extra_option3": (CAPTIONFORGE_EXTRA_OPTIONS_CHOICES, {"default": "", "advanced": True}),
                "extra_option4": (CAPTIONFORGE_EXTRA_OPTIONS_CHOICES, {"default": "", "advanced": True}),
                "extra_option5": (CAPTIONFORGE_EXTRA_OPTIONS_CHOICES, {"default": "", "advanced": True}),
                "extra_option6": (CAPTIONFORGE_EXTRA_OPTIONS_CHOICES, {"default": "", "advanced": True}),
                "person_name": ("STRING", {"default": "", "multiline": False, "advanced": True}),
            ...
            "optional": {
                ...
                "extra_options": ("CAPTIONFORGE_EXTRA_OPTIONS", {"tooltip": "Optional shared CaptionForge Extra Options payload. Overrides local extra_option widgets when non-empty."}),
            },
        }
    ...
    def caption(self, model, memory_mode, input_path, recursive, filename_glob, prompt_mode, prompt_preset, caption_type, caption_length, extra_option1, extra_option2, extra_option3, extra_option4, extra_option5, extra_option6, person_name, custom_prompt, max_new_tokens, num_beams, temperature, top_p, top_k, repetition_penalty, captions_per_image, seed, max_size, output_dir, write_txt, write_jsonl, also_jsonl, write_run_config, jsonl_filename, overwrite, backup_existing, dry_run, limit, skip_existing_txt, skip_existing_jsonl_images, prefix, suffix, forbidden_phrases, replace_pairs, keep_loaded, download_probe_only, image=None, captionforge_run_config=None, extra_options=None):
        if download_probe_only:
            result = probe_registry_model_download(model, JLC_SMOLVLM_MODEL_ROOT)
            return (captionforge_run_config, result, "", "")
        local_extra_options = _extra_options_from_widgets(extra_option1, extra_option2, extra_option3, extra_option4, extra_option5, extra_option6)
        ...
        if prompt_mode == "caption_template":
            prompt = build_caption_prompt(caption_type, caption_length, effective_extra_options, effective_person_name, dialect="smolvlm")
        else:
            prompt = resolve_prompt(prompt_preset, custom_prompt)
        run_plan = expand_captionforge_runs(captionforge_run_config, model_key="smolvlm", widget_captions_per_image=int(captions_per_image), widget_seed=int(seed), widget_temperature=float(temperature), widget_top_p=float(top_p), widget_top_k=int(top_k), widget_max_new_tokens=int(max_new_tokens), widget_max_size=int(max_size), widget_trigger_word="", widget_output_dir=output_dir, widget_input_path=input_path, widget_recursive=bool(recursive), widget_filename_glob=filename_glob)
        run_plan_connected = bool(captionforge_run_config)
        if run_plan_connected and not run_plan:
            status = "[CaptionForge] JLC SmolVLM Caption disabled by Pipeline Planner."
        ...
        if pil_images:
            engine.load()
            fallback_dir = Path(folder_paths.get_output_directory()) / "jlc_smolvlm_caption"
            ...
            if write_run_config and not input_path:
                write_run_config_json(image_output_dir / f"jlc_smolvlm_caption_run_config_{timestamp()}.json", engine.build_run_config(), dry_run=bool(dry_run))
...
NODE_CLASS_MAPPINGS = {"JLC_SmolVLMCaption": JLC_SmolVLMCaption}
NODE_DISPLAY_NAME_MAPPINGS = {"JLC_SmolVLMCaption": " JLC SmolVLM Caption"}
