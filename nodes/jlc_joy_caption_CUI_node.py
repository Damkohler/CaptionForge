"""
JLC Joy Caption — ComfyUI Node Wrapper
...
- Node Purpose
    - The **JLC Joy Caption** node provides a single-node ComfyUI frontend for
      JoyCaption/LLaVA-family image captioning inside CaptionForge.
    ...
    - This file is the **ComfyUI-facing wrapper**, not the reusable captioning
      engine. It is responsible for:
            ...
            • JoyCaption-native template prompt controls
...
- CaptionForge Pass Role
    ...
    - The Joy node writes records compatible with downstream CaptionForge
...
- Node Workflow Model
    ...
    - For direct IMAGE input with no output directory, sidecar files are written
      to:
            ComfyUI/output/jlc_joy_caption/
...
- Prompting Model
    ...
    - In `joy_template` mode:

            prompt = Joy caption_type
                   + caption_length
                   + optional extra instructions
                   + optional person_name substitution

    - This preserves CaptionForge-style prompt presets while also exposing
      JoyCaption-native prompt-template behavior.

- Model and Dependency Notes
    - JoyCaption/LLaVA repositories are multi-file Hugging Face model directories,
...
- Design Philosophy
    - This node preserves JoyCaption as one independent captioning voice inside
      CaptionForge rather than treating any single model as canonical.
...
- ⚠️ Development Status
    ...
    - The UI, prompt templates, model registry, memory modes, and output audit
      fields may evolve as the multi-pass CaptionForge pipeline matures.
...
MANIFEST = {
    "name": "JLC Joy Caption",
    "version": (1, 1, 0),
    "author": "J. L. Córdova",
    "description": (
        "Single-node ComfyUI frontend for JoyCaption/LLaVA-family image captioning inside "
        "CaptionForge Pass A. Provides widget schema, direct IMAGE tensor handling, "
        "file/folder routing, CaptionForge prompt preset/file/custom prompt selection, "
        "JoyCaption-native template controls, fixed models/LLM integration, download-probe "
        "exposure, TXT sidecar controls, JSONL audit output, run-config export, and "
        "caption/JSONL string returns. Delegates reusable Joy/LLaVA model loading, generation, "
        "8-bit memory-efficient loading, ComfyUI model-management bridge behavior, cleanup, "
        "cache integration, batch traversal, and audit-record creation to "
        "jlc_joy_caption_engine.py so Joy captions can contribute auditable evidence to "
        "downstream model-agnostic claim extraction and consensus refinement passes."
    ),
}
...
#   ComfyUI/models/LLM/JLC_JoyCaption/<model-folder>
# -------------------------------------------------------------------------
...
def _parse_replace_pairs(value: str) -> list[tuple[str, str]]:
    """
    ComfyUI-friendly replacement format:
      old=>new
...
class JLC_JoyCaption:
    """
    Single-node JoyCaption/LLaVA captioner.

    This node mirrors the JLC Qwen Caption ComfyUI interface where possible,
    but delegates Joy-specific model handling to `jlc_joy_caption_engine.py`.
    """
        ...
        return {
            "required": {
                ...
                "person_name": (
                    "STRING",
                    {
                        "default": "",
                        "multiline": False,
                        "tooltip": "Replacement value for the {name} placeholder in matching Joy extra options.",
                        "advanced": True,
                    },
                ),
                ...
                "custom_prompt": (
                    "STRING",
                    {
                        ...
                        "tooltip": (
                            ...
                            "not currently overridden by the CaptionForge Pipeline Planner."
                        ),
                    },
                ),
            ...
            "optional": {
                ...
                "extra_options": (
                    ...
                    {
                        "tooltip": (
                            "Optional shared CaptionForge Extra Options payload. When connected and "
                            "non-empty, it overrides the local extra_option widgets for prompt-template mode."
                        ),
                    },
                ),
            },
    ...
    def caption(
        ...
        use_caption_template = prompt_mode in {"caption_template", "joy_template"}

        prompt = resolve_prompt(
            prompt=custom_prompt if not use_caption_template else "",
            prompt_file=prompt_file if not use_caption_template else "",
        ...
        if run_plan_connected and not run_plan:
            status = "[CaptionForge] Joy Caption disabled by Pipeline Planner."
            print(status)
        ...
        if pil_images:
            ...
            fallback_dir = Path(folder_paths.get_output_directory()) / "jlc_joy_caption"
            ...
            if write_run_config and not input_path:
                write_run_config_json(
                    image_output_dir / f"jlc_joy_caption_run_config_{timestamp()}.json",
...
NODE_CLASS_MAPPINGS = {
    "JLC_JoyCaption": JLC_JoyCaption,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "JLC_JoyCaption": "\u2003JLC Joy Caption",
}
