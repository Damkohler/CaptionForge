...
Compact direct-IMAGE / Run Plan captioning frontend for Qwen-family VLMs.
...
The reusable model logic lives in jlc_qwen_caption_engine.py.
"""
...
MANIFEST = {
    ...
    "author": "J. L. Córdova",
    "description": (
        "Compact ComfyUI frontend for Qwen-family vision-language captioning. "
        "In standalone mode it behaves as a small direct-IMAGE caption node. "
        "When a CaptionForge Run Plan is connected, it becomes a production Pass A "
        "evidence node: it uses shared input_path/output_dir routing, per-run seeds "
        "and sampling schedules, writes TXT audit sidecars, appends common JSONL "
        "evidence records, and still returns readable captions in the UI."
    ),
}
